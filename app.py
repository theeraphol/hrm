...
def render_page(body_html: str, title: str = "Service Queue"):
    ...
    <body class=\"min-h-screen flex items-center justify-center bg-gradient-to-tr from-indigo-500 via-sky-500 to-emerald-500 text-gray-800\">
      <div class=\"bg-white/70 backdrop-blur-md shadow-2xl rounded-xl p-8 w-full max-w-lg\">
        {body_html}
      </div>
    </body>
    </html>
...
def book():
    ...
    queues.append(queue)
    next_queue += 1
    success_html = f"<p class='bg-green-100 border border-green-400 text-green-700 px-4 py-2 rounded'><i class='fa-solid fa-circle-check mr-1'></i>จองคิวสำเร็จ หมายเลขคิว {queue['number']}</p>"
...
def login():
    ...
    login_html = '<p><i class="fa-solid fa-right-to-bracket mr-1"></i>Redirect to your SSO provider and return with ?user=ID</p>'
    return render_page(login_html, title="เข้าสู่ระบบ")
...
def staff():
    if not sso_authenticated():
        return redirect(url_for('service_queue.login'))
    queue_list = ''.join(
        f"<li class='py-2 flex items-center gap-2'><i class='fa-solid fa-user'></i>{q['number']} - {q['id_card']}</li>" for q in queues
    )
    staff_html = f"""
    <h1 class=\"text-2xl font-bold mb-4\"><i class=\"fa-solid fa-list mr-2\"></i>รายการคิว</h1>
    <ul class=\"divide-y divide-gray-200 mb-3\">{queue_list}</ul>
    """
...
def call_next():
    ...
    <p><i class='fa-solid fa-bell mr-1'></i>{message}</p>
    """
